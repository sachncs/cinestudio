...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-card border border-border shadow-warm transition ease-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-200',
  {
    variants: {
      side: {
...
const SheetFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn('flex flex-col-reverse p-6 sm:flex-row sm:justify-end sm:space-x-2', className)}
    {...props}
  />
);
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn('text-lg font-display font-semibold text-foreground', className)}
    {...props}
  />
));
